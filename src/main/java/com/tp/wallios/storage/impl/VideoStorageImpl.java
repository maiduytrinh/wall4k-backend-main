...
public class VideoStorageImpl implements VideoStorage {
    private static final Logger LOG = TPLogger.getLogger(VideoStorageImpl.class);
    public static Pageable<Data> getPageable(Pageable<Data> pageable) {
        ...
        try {
            List<Data> listData = Utils.mysqlMainUtility.executeQuery(String.format("SELECT * FROM videos where isApproved = 1 LIMIT %s, %s",pageable.getOffset(), pageable.getLimit()), Data.class);
